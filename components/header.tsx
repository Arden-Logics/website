    ...
    return (
        <>
            <header
                ...
                <div
                    className={cn(
                        'h-32 absolute inset-x-0 top-0 z-50 border-b-4 border-foreground/20 ring-1 ring-transparent transition-all duration-300 pb-8 max-lg:pb-0',
                        'in-data-scrolled:border-foreground/20 in-data-scrolled:border-b-2 in-data-scrolled:bg-background/75 in-data-scrolled:backdrop-blur in-data-scrolled:pb-8 max-lg:in-data-scrolled:pb-0',
                        ...
                        'max-lg:in-data-[state=active]:h-screen max-lg:in-data-[state=active]:bg-background/75 max-lg:in-data-[state=active]:backdrop-blur max-lg:h-24 max-lg:overflow-hidden max-lg:border-b bg-background pt-4'
                    )}>
                    <div className="w-full px-8 sm:px-12 lg:px-24 xl:px-32">
                        <div className="relative flex flex-wrap items-center justify-between lg:py-4">
                            <div className="flex justify-between gap-8 max-lg:h-24 max-lg:w-full max-lg:border-b">
                                <Link
                                    href="/"
                                    aria-label="home"
...
const MobileMenu = ({ closeMenu }: { closeMenu: () => void }) => {
    return (
        <nav
            role="navigation"
            className="w-full overflow-x-hidden overflow-y-auto max-h-[calc(100vh-12rem)] [--color-border:--alpha(var(--color-foreground)/5%)] [--color-muted:--alpha(var(--color-foreground)/5%)]">
            <Accordion
                type="single"
                collapsible
                className="**:hover:no-underline mt-0.5 space-y-0.5">
                <AccordionItem
                    value="services"
        ...
        <NavigationMenu
            ...
            <NavigationMenuList className="gap-3">
                ...
                <NavigationMenuItem value="solutions">
                    ...
                    <NavigationMenuContent className="mt-8 origin-top pb-14 pt-5 shadow-none ring-0">
                        <div className="w-[1100px] grid grid-cols-4 gap-3 p-4 pl-2">
                            {SOLUTIONS.map((category, categoryIndex) => {
                                if (category.title === 'By Sector') {
                                    const col1 = category.items.slice(0, 4);
                                    const col2 = category.items.slice(4, 8);
                                    const col3 = category.items.slice(8, 12);
                                    return (
                                        <React.Fragment key={categoryIndex}>
                                            <div>
                                                <h3 className="text-sm font-semibold text-black mb-4 pl-0">{category.title}</h3>
